...
from Air_Quality import Update_Air_Status
##################################################################
...
def App_Main():
    ...
    if Temprature != 0:
        ...
        if sys_status["internet_connection"] == "Connected":
            Google_Sheets_Runnable(Temprature, Humidity, Air_sts["AirQuality"])
...
def Sensors_Handler():
    ...
    #print("OUT of AIR request",Air_Quality)
    values = Air_Quality.values()
    total_Air_Quality = sum(values)
    Ammonia_sts["Ammonia"]=(total_Air_Quality)
    Air_sts["AirQuality"]=(total_Air_Quality)
    print("Air_Quality")
    print(total_Air_Quality)
    ...
    '''
    values = Temprature.values()
    total_Tamp_Correct_Values = sum(values)
    Room_temp_sts["Room_temp"] = (total_Tamp_Correct_Values )
