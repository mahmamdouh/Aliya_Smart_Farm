...
import collections
import re
from DTC import Sens_Data_Range_DIAG_Update
##################################################################
...
def Correct_factor_runable(reading , Factor_value):
	...
	while True:
		...
		if deleted_reading != "":
			...
			break
	DTC_Update(Rading_correction)
	return Rading_correction
	#print("############################################################")	


def DTC_Update(Readings):
	Corrupted_Data=Get_Sensor_Out_Of_Range(Readings)
	x=len(Corrupted_Data)
	for i in range(x):
		Sens_Data_Range_DIAG_Update(Corrupted_Data[i])
	return
