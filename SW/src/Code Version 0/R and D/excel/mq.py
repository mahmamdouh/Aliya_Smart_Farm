...
class MQ():
    ...
    RO_CLEAN_AIR_FACTOR          = 3.85     # RO_CLEAR_AIR_FACTOR=(Sensor resistance in clean air)/RO,
    ...
    def __init__(self, Ro=10, analogPin=0):
        ...
        self.LPGCurve = [0.41,1,-2.48]    # two points are taken from the curve. 
                                            # with these two points, a line is formed which is "approximately equivalent"
                                            # to the original curve. 
                                            # data format:{ x, y, slope}; point1: (lg10 0.415), point2: (lg200, -0.1023) 
        self.COCurve = [1.1,0.4397,-0.257]     # two points are taken from the curve. 
                                            # with these two points, a line is formed which is "approximately equivalent" 
                                            # to the original curve.
                                            # data format:[ x, y, slope]; point1: (lg200, 0.72), point2: (lg10000,  0.15)
        self.SmokeCurve =[1,0.154,-0.289]   # two points are taken from the curve. 
                                            # with these two points, a line is formed which is "approximately equivalent" 
                                            # to the original curve.
    ...
    def MQPercentage(self):
        ...
        val["GAS_ammonia"]  = self.MQGetGasPercentage(read/self.Ro, self.GAS_LPG)
        val["CO"]       = self.MQGetGasPercentage(read/self.Ro, self.GAS_CO)
        val["Asitona"]    = self.MQGetGasPercentage(read/self.Ro, self.GAS_SMOKE)
        return val
    ...
    def MQCalibration(self, mq_pin):
        ...
        for i in range(self.CALIBARAION_SAMPLE_TIMES):          # take multiple samples
            ...
            val += self.MQResistanceCalculation(ADC_READ)
